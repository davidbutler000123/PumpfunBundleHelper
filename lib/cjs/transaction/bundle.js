        ...
        while(nTryCount < 10) {
            try {
                ...
                for (let i = 0; i < bundleTranasction.length; i++) {
                    bundleTranasction[i].txn.message.recentBlockhash = blockhash;
                    // (0, sign_tx_1.signTransaction)(bundleTranasction[i]);
                    bundleTranasction[i].txn.sign(bundleTranasction[i].singer)
                    realForBundle.push(bundleTranasction[i].txn);
                    txnSizes.push(bundleTranasction[i].txn.serialize().length)
                }
                
                console.log('txnSizes: ', txnSizes)
